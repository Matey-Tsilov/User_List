...
const UserEdit = ({setUsers, user, closeHandler}) => {
    ...
    const updateCurrentUser = (e) => {
     ...
     const updatedUser = {firstName, lastName, email, phoneNumber, imageUrl, address}

     userService.updateUser(user._id, updatedUser)
     .then(res => setUsers(users => {
        const upUser = users.find(x => x._id == res._id)
        const index = users.indexOf(upUser)
        users.splice(index, 1, res)
        closeHandler()
        return users
     }))
